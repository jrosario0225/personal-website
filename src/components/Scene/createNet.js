...
function createNet(scene) {
    ...
    // Left pole
    const poleGeo = new THREE.CylinderGeometry(0.06, 0.06, netHeight + 0.04, 16)
    const poleMat = new THREE.MeshPhongMaterial({ color: 0xff2200 })
    const leftPole = new THREE.Mesh(poleGeo, poleMat)
    leftPole.position.set(-netWidth / 2, netY + netHeight / 2 + 0.2, netZ)
    netGroup.add(leftPole)

    // Right pole
    const rightPole = new THREE.Mesh(poleGeo, poleMat)
    rightPole.position.set(netWidth / 2, netY + netHeight / 2 + 0.2, netZ)
